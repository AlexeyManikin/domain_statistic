...
class ShowStatistic(object):
    ...
    def get_info_to_as_now(self, as_number, date_start):
        """
        Сколько доменов пришло на нашу AS и откада
        :param as_number:
        :param date_start:
        :param date_end:
        :return:
...
ORDER BY t.count;""".format(as_number, date_start)
    ...
    def show(self):
        info = self.get_info_to_as_now(198610, '2018-08-01')
        sorted(info, key=by_count)
        x = PrettyTable()
        x.field_names = ["AS", "Desriptions", "Count"]
        for row in info:
            x.add_row([row['as_number'], row['as_description'], row['count']])
        print(x)
